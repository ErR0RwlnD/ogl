...
int initWindow(void)
{
    ...
    window = glfwCreateWindow(window_width, window_height, "Jinzhou Yu", NULL, NULL);
    if (window == NULL)
    {
...
void uniformCircle(Vertex *Vertices, GLushort *Indices)
{
    for (size_t i = 0; i < IndexCount; i++)
    {
        Vertices[i] = {{sin((float)M_PI / IndexCount * i * 2), cos((float)M_PI / IndexCount * i * 2), 0.0f, 1.0f}, {(float)rand() / RAND_MAX, (float)rand() / RAND_MAX, (float)rand() / RAND_MAX, 1.0f}};
        Indices[i] = i;
    }
}
...
void createObjects(void)
{
    ...
    uniformCircle(Vertices, Indices);
